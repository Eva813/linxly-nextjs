...
const PromptDialog = () => {
  // 改用 usePromptStore 取得 UI 狀態與操作方法
  const { isDialogOpen, setIsDialogOpen, matchedPrompt, setMatchedPrompt } = usePromptStore();
  ...
  useEffect(() => {
    if (matchedPrompt.content) {
      const parseContent = () => {
        ...
        div.innerHTML = matchedPrompt.content;
        ...
        formTextElements.forEach((span, index) => {
          ...
          const spanIndex = matchedPrompt.content.indexOf(spanOuterHTML, currentPos);

          // 如果 span 前面還有 text，則添加到 newSegments 中
          if (spanIndex > currentPos) {
            const beforeText = matchedPrompt.content.substring(currentPos, spanIndex);
            newSegments.push({
              type: 'text',
        ...
        // 添加剩餘的 text
        if (currentPos < matchedPrompt.content.length) {
          newSegments.push({
            type: 'text',
            content: stripHtml(matchedPrompt.content.substring(currentPos))
          });
        }
  ...
  const handleInsert = () => {
    if (matchedPrompt.targetElement) {
      let finalContent = matchedPrompt.content;
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogContent className="bg-white">
        ...
        <div className="p-4">
          <div className="mb-4">
            <h3 className="text-lg font-medium">Insert Prompt</h3>
