...
export async function GET(req: Request) {
  try {
    ...
    const result = await Promise.all(
      filteredFolders.map(async (folderDoc) => {
        ...
        console.log(
          `API: Processing folder ${folderId} (${folder.name}): found ${folderPrompts.length} prompts`
        );
        ...
        if (folderPrompts.length === 0) {
          console.log(
            `API: No prompts found for folder ${folderId}, trying direct query...`
          );
          ...
          if (!directQuery.empty) {
            console.log(
              `API: Direct query found ${directQuery.docs.length} prompts for folder ${folderId}`
            );
            const directPrompts = directQuery.docs.map(
              mapFirestoreDocToPromptData
            );
        ...
        const sortedPrompts = finalPrompts.sort(
...
export async function POST(req: Request) {
  try {
    ...
    if (
      typeof body.promptSpaceId !== 'string' ||
      body.promptSpaceId.trim().length === 0
    ) {
      return NextResponse.json(
