...
const Sidebar = () => {
  ...
  const { addingFolder, addingPrompt } = useSidebarStore();
  const { handleAddFolder, handleAddPrompt } = useSidebarActions();
  ...
  return (
    <div className="p-4 border-r border-gray-300 h-full flex flex-col">
      <div className="grid grid-cols-2 gap-x-4 mb-4 sticky top-0 bg-white dark:bg-gray-900 z-10">
        ...
        >
          {addingFolder ? <FaSpinner className="animate-spin" /> : <FaFolderPlus />}
          Add Folder
        </Button>
        <Button
          className="h-8 dark:text-third"
